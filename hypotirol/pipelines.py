...
class HypotirolPipeline:

    # Database setup
    conn = sqlite3.connect('hypotirol.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hypotirol`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hypotirol WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hypotirol (date, title, link, content)"
